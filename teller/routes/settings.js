...
router.get("/api/settings", async (req, res) => {
  // Embedded under the unified shell, digest emails are delivered in-process
  // via the wired Per-sistant pool (no webhook URL/secret needed), so the UI
...
router.patch("/api/settings", async (req, res) => {
  ...
  try {
    ...
    if (partner_name !== undefined) {
      const v = typeof partner_name === "string" ? partner_name.trim().slice(0, 50) : null;
      updates.push("partner_name = $" + idx++); values.push(v || null);
    }
    ...
    // Weekly digest (S2)
    if (req.body.weekly_digest_enabled !== undefined) {
    ...
    // of remaining life. Bounded 5 min … 7 days (10080 min) so a typo can't
    // silently disable auth or lock the user out from too-tight an idle.
    ...
    // Tell the shell auth module to drop its 60s read-cache so the new idle
    // window applies on the very next request. No-op when running standalone
    // (no shell wired the invalidator). Defensive try in case it throws.
...
// Returns per-source { timestamp, age_seconds, stale, level } where level is
// 'fresh' (<6h), 'aging' (6-24h), or 'stale' (>24h or never synced). The nav
// badge uses these thresholds to show green/yellow/red and previously had to
// duplicate the threshold values; exposing `level` directly keeps the API
// and UI consistent without requiring callers to repeat threshold math.
router.get("/api/data-freshness", async (_req, res) => {
  try {
    ...
    const now = Date.now();
    const FRESH_SEC = 6 * 60 * 60;       // <6h → green
    const STALE_SEC = 24 * 60 * 60;       // >24h → red
    function ageInfo(ts) {
  ...
  try {
    ...
    try {
      ...
      const jr = await pool.query(
        "SELECT job_name, last_run_at, last_status FROM job_runs WHERE job_name != '_watchdog' ORDER BY job_name"
      );
      const nowMs = Date.now();
      jobs = jr.rows.map(j => ({
        job: j.job_name,
        last_run_at: j.last_run_at,
        stale: j.last_run_at ? (nowMs - new Date(j.last_run_at).getTime()) > jobHealth.thresholdMs(j.job_name) : false,
      }));
      const staleJobs = jobs.filter(j => j.stale).map(j => j.job);
      if (staleJobs.length) {
        issues.push({ severity: "warning", message: `Scheduled job(s) haven't run recently: ${staleJobs.join(", ")} — server likely asleep (check keep-alive).` });
      }
    } catch (e) { /* job_runs not migrated yet — omit */ }
